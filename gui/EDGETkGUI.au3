; #INDEX# =======================================================================================================================
; Title .........: EDGETk Graphical User Interface
; AutoIt Version : 3.3.14.5
; Language ......: English
; Description ...: Handler/Wrapper for EDGETk Golang Tool Functions.
		...
		Switch $nMsg
			...
			Case $Button1 ; Generate Keypair (DH)
				...
				If $sRead = "" and $rRead = "" Then
					$CMD = "edgetk -keygen -info Keypair -algorithm " & $iAlgorithm & " | edgetk -util unix2dos > " & @TempDir & "keypair.txt"
				ElseIf $sRead not = "" and $rRead = "" Then
					$CMD = "edgetk -keygen -info Keypair -algorithm " & $iAlgorithm & " -key """ & $sRead  & """ | edgetk -util unix2dos > " & @TempDir & "keypair.txt"
				Else
					$CMD = "edgetk -keygen -info Keypair -algorithm " & $iAlgorithm & " | edgetk -util unix2dos > " & @TempDir & "keypair.txt"
				EndIf
...
;				FileDelete(@TempDir & "keypair.txt")
				$CMD = "edgetk -shred " & @TempDir & "keypair.txt -iter 5"
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
			...
			Case $Button2
				...
				$CMD = "echo [Keypair] > " & @TempDir & "keypair.txt & edgetk -pkeyutl derive -algorithm " & $iAlgorithm  & " -key " & $sRead & " -pub " & $xRead & " | edgetk -util unix2dos >> " & @TempDir & "keypair.txt"
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
...
;				FileDelete(@TempDir & "keypair.txt")
			...
			Case $Button8 ; Sign
				...
				If $radioval = "File" Then
					$CMD = "echo [Signature] > " & @TempDir & "Sign.txt & edgetk -sign -algorithm " & $iAlgorithm  & " -md " & $iHash  & " -key " & $sRead & " < " & $sFile & " | edgetk -util unix2dos >> " & @TempDir & "Sign.txt"
					RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				ElseIf $radioval = "String" Then
					$CMD = "echo [Signature] > " & @TempDir & "Sign.txt & busybox echo -n " & $xRead & " | edgetk -sign -algorithm " & $iAlgorithm  & " -md " & $iHash  & " -key " & $sRead & " | edgetk -util unix2dos >> " & @TempDir & "Sign.txt"
					RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				EndIf
			...
			Case $Button12 ; Verify
				...
				EndSelect
				If $radioval = "File" Then
					$CMD = "edgetk -verify -key " & $rRead & " -signature " & $yRead & " -algorithm " & $iAlgorithm  & " -md " & $iHash  & " < " & $sFile & " > " & @TempDir & "Signature.txt"
					RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				ElseIf $radioval = "String" Then
					$CMD = "busybox echo -n " & $xRead & " | edgetk -verify -key " & $rRead & " -signature " & $yRead & " -algorithm " & $iAlgorithm  & " -md " & $iHash  & " > " & @TempDir & "Signature.txt"
					RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				EndIf
...
;						$iAlgorithm = "numsp256d1"
				...
				$CMD = "busybox echo -n """ & $PlainText & """ > " & @TempDir & "Plaintext.txt & edgetk -pkeyutl enc -algorithm " & $iAlgorithm & " -key " & $PublicKey & " < " & @TempDir & "Plaintext.txt | edgetk -util hexdec | edgetk -util b32enc+ > " & @TempDir & "Ciphertext.txt"
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
...
;				FileDelete("Key.txt")
			...
			Case $Button28 ; Encrypt with Symmetric Key
				...
				If $radioval = "File" Then
					If GUICtrlRead($Checkbox1) = $GUI_CHECKED Then
						$CMD = "edgetk -crypt enc -mode " & $Mode & " -info " & $yRead & " -cipher " & $iAlgorithm & " -key """ & $xRead & """ < " & $sFile & " > " & $xFile
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					Else
						$CMD = "edgetk -crypt enc -mode " & $DMode & " -cipher " & $iAlgorithm & " -key " & $xRead & " -iv """ & $aRead & """ < " & $sFile & " > " & $xFile
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					EndIf
					MsgBox($MB_SYSTEMMODAL, "", "Encryption" & @CRLF & " Done")
				ElseIf $radioval = "String" Then
					If GUICtrlRead($Checkbox1) = $GUI_CHECKED Then
						$CMD = "busybox echo -n " & $sRead & " | edgetk -crypt enc -mode " & $Mode & " -info " & $yRead & " -cipher " & $iAlgorithm & " -key """ & $xRead & """ | edgetk -util " & $command & " > " & @TempDir & "Ciphertext.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					Else
						$CMD = "busybox echo -n " & $sRead & " | edgetk -crypt enc -mode " & $DMode & " -cipher " & $iAlgorithm & " -key """ & $xRead & """ -iv """ & $aRead & """ | edgetk -util " & $command & " > " & @TempDir & "Ciphertext.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					EndIf
					$Result = FileRead(@TempDir & "Ciphertext.txt")
					$LineFeedStrip = StringReplace($Result, @LF, "")
...
;					FileDelete(@TempDir & "Ciphertext.txt")
			...
			Case $Button29 ; Decrypt with Symmetric Key
				...
				If $radioval = "File" Then
					If GUICtrlRead($Checkbox1) = $GUI_CHECKED Then
						$CMD = "edgetk -crypt dec -mode " & $Mode & " -info " & $yRead & " -cipher " & $iAlgorithm & " -key " & $xRead & " < " & $sFile & " > " & $xFile
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					Else
						$CMD = "edgetk -crypt dec -mode " & $DMode & " -key " & $xRead & " -cipher " & $iAlgorithm & " -iv """ & $aRead & """ < " & $sFile & " > " & $xFile
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					EndIf
...
;					FileDelete(@TempDir & "Plaintext.txt")
			...
			Case $Button34 ; Compute Digests
				...
				If $radioval = "File" Then
					If GUICtrlRead($Checkbox2) = $GUI_CHECKED Then
						$CMD = "echo [Digest] > " & @TempDir & "Digest.txt & edgetk -mac hmac -md " & $iHash & " -key """ & $rRead & """ < " & $sFile & " >> " & @TempDir & "Digest.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
						$id = "MAC-" & $idHash
					...
					Else
						$CMD = "edgetk -digest - -md " & $iHash & " -key " & $rRead & " -key """ & $rRead & """ < " & $sFile & " > " & @TempDir & "Digest.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				...
				ElseIf $radioval = "String" Then
					If GUICtrlRead($Checkbox2) = $GUI_CHECKED Then
						$CMD = "echo [Digest] > " & @TempDir & "Digest.txt & busybox echo -n " & $sRead & " | edgetk -mac hmac -md " & $iHash & " -key """ & $rRead & """ >> " & @TempDir & "Digest.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
						$id = "MAC-" & $idHash
						$Result = IniRead(@TempDir & "Digest.txt", "Digest", $id, "Error.")
						GUICtrlSetData($Edit16, $Result)
						FileDelete(@TempDir & "Digest.txt")
					Else
						$CMD = "busybox echo -n " & $sRead & " | edgetk -digest - -md " & $iHash & " -key """ & $rRead & """ > " & @TempDir & "Digest.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
						$Result = FileRead(@TempDir & "Digest.txt")
						GUICtrlSetData($Edit16, $Result)
						FileDelete(@TempDir & "Digest.txt")
			...
			Case $Button35 ; Check Digests
				...
				If $radioval = "File" Then
					If GUICtrlRead($Checkbox2) = $GUI_CHECKED Then
						$CMD = "edgetk -mac hmac -md " & $iHash & " -key """ & $rRead & """ -signature " & $xRead & " < " & $sFile & " > " & @TempDir & "Digest.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
						Local $sFileRead = FileRead(@TempDir & "Digest.txt")
						MsgBox($MB_SYSTEMMODAL, "", "Digest Verification:" & @CRLF & $sFileRead)
						FileDelete(@TempDir & "Digest.txt")
					Else
						$CMD = "edgetk -digest - -md " & $iHash & " < """ & $sFile & """ -key """ & $rRead & """ > " & @TempDir & "Digest.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
						$Result = FileRead(@TempDir & "Digest.txt")
				...
				ElseIf $radioval = "String" Then
					If GUICtrlRead($Checkbox2) = $GUI_CHECKED Then
						$CMD = "busybox echo -n " & $sRead & " | edgetk -mac hmac -md " & $iHash & " -key """ & $rRead & """ -signature " & $xRead & " > " & @TempDir & "Digest.txt"
						RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
						Local $sFileRead = FileRead(@TempDir & "Digest.txt")
						MsgBox($MB_SYSTEMMODAL, "", "Digest Verification:" & @CRLF & $sFileRead)
						FileDelete(@TempDir & "Digest.txt")
			...
			Case $Button36
				...
				Switch GUICtrlRead($Combo5)
					...
					Case "BLAKE2s256"
						...
						EndIf
				EndSwitch
				$CMD = "edgetk -rand -bits " & $length & " > " & @TempDir & "Key.txt"
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				$Key = FileRead(@TempDir & "Key.txt")
				$LineFeedStrip = StringReplace($Key, @LF, "")
...
;				FileDelete(@TempDir & "Key.txt")
			...
			Case $Button39 ; KDF
				...
				If GUICtrlRead($Checkbox3) = $GUI_CHECKED Then
					$CMD = "edgetk -kdf pbkdf2 -md " & $iHash & " -bits " & $bRead & " -key " & $rRead & " -salt " & $xRead & " -iter " & $iRead & " > " & @TempDir & "KDF.txt"
					RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					$id = "MAC-" & $idHash
					$Result = FileRead(@TempDir & "KDF.txt")
					GUICtrlSetData($Edit19, $Result)
;					FileDelete("KDF.txt")
					$CMD = "edgetk -shred " & @TempDir & "KDF.txt -iter 5"
					RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				Else
					$CMD = "edgetk -kdf hkdf -md " & $iHash & " -bits " & $bRead & " -key " & $rRead & " -salt " & $xRead & " -info " & $yRead & " > " & @TempDir & "KDF.txt"
					RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
					$Result = FileRead(@TempDir & "KDF.txt")
					GUICtrlSetData($Edit19, $Result)
;					FileDelete("KDF.txt")
			...
			Case $Button40
				...
				Else
					$CMD = "busybox echo -n """ & $rRead & """ | edgetk -tcp send -pub """ & $tRead & ":" & $wRead & """ -algorithm " & $iAlgorithm & ""
				EndIf
;				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
			...
			Case $Button41
				...
				Else
					$CMD = "edgetk -tcp dump -pub " & $hRead & " -algorithm " & $iAlgorithm & " > dump.txt"
				EndIf
				RunWait(@ComSpec & " /c " & $CMD, "", 6)
				$Result = FileRead("dump.txt")
				GUICtrlSetData($Edit20, $Result)
;				FileDelete("dump.txt")
				$CMD = "edgetk -shred " & @TempDir & "dump.txt -iter 5"
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)

			Case $Button42
				$CMD = "edgetk -tcp ip > " & @TempDir & "ip.txt"
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
...
;				FileDelete(@TempDir & "Key.txt")
			...
			Case $Button54 ; Generate IV
				...
				If GUICtrlRead($Combo10) = "MAC" Then
					...
					EndSwitch
				EndIf
				$CMD = "edgetk -rand -bits " & $iv & " > " & @TempDir & "Key.txt"
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
				$Key = FileRead(@TempDir & "Key.txt")
				$LineFeedStrip = StringReplace($Key, @LF, "")
...
;				FileDelete(@TempDir & "Key.txt")
			...
			Case $Button47 ; Generate MAC
				...
				$id = "MAC-" & $idCipher
				If $radioval = "File" Then
					$CMD = "echo [MAC] > " & @TempDir & "MAC.txt & edgetk " & $iCommand & " " & $iAlgorithm & " -key """ & $rRead & """ -iv """ & $xRead & """ < " & $sFile & " >> " & @TempDir & "MAC.txt"
				Else
					$CMD = "echo [MAC] > " & @TempDir & "MAC.txt & busybox echo -n """ & $sRead & """ |edgetk " & $iCommand & " " & $iAlgorithm & " -key """ & $rRead & """ -iv """ & $xRead & """ >> " & @TempDir & "MAC.txt"
				EndIf
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
			...
			Case $Button48
				...
				EndSelect
				If $radioval = "File" Then
					$CMD = "edgetk " & $iCommand & " " & $iAlgorithm & " -key """ & $rRead & """ -iv """ & $xRead & """ -signature """ & $yRead & """ < " & $sFile & " > " & @TempDir & "Check.txt"
				Else
					$CMD = "busybox echo -n """ & $sRead & """ |edgetk " & $iCommand & " " & $iAlgorithm & " -key """ & $rRead & """ -iv """ & $xRead & """ -signature """ & $yRead & """ > " & @TempDir & "Check.txt"
				EndIf
				RunWait(@ComSpec & " /c " & $CMD, "", @SW_HIDE, 6)
